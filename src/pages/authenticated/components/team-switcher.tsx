...
const groups = [
	{
		...
		teams: [
			...
			},
			{
				...
				to: _path(ROUTES.user, ROUTES.dashboard),
			},
			{
				label: "Phoenix Corp.",
				value: "phoenix-corp",
		...
		<Popover
			...
			<PopoverTrigger asChild>
				<Button
					...
					className={cn(
						`h-auto w-full justify-between border-none px-2 py-3 ${
							open && "bg-accent"
						}`,
						className,
					)}>
				...
				<Command>
					<CommandList>
						<CommandGroup>
							...
							<CommandItem className="flex items-center py-2">
								<span className="align-middle leading-4">Account Settings</span>
							</CommandItem>
							<CommandItem className="flex items-center py-2">
								<span className="align-middle leading-4">
									Device Management
					...
					<CommandList className="border-accent">
						<CommandInput
							className="border-0 border-accent"
							placeholder="Search team..."
						/>
						<CommandEmpty>No team found.</CommandEmpty>
						<ScrollArea>
							<ScrollAreaViewport className="max-h-max">
								{groups.map((group) => (
									<CommandGroup
										className="p-1"
										key={group.label}
